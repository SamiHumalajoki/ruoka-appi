      ...
      {isEmpty ? (
        <>
          ...
          <button onClick={e => { e.stopPropagation(); onArvo(); }} style={{
            ...
            <Icon name="dice" size={15} color="#fff" /> Arvo
          </button>
        </>
      ) : rolling ? (
        <>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 15, color: C.ink, marginTop: 2, fontWeight: 600, opacity: 0.8 }}>{rollName || '…'}</div>
          </div>
        </>
      ) : (
        <>
...
function DayCard({ idx, day, date, isToday, plan, recipes, rolling, rollNames, onArvo, onOpen, onReroll }) {
  return (
    <div style={{
      ...
      boxShadow: isToday ? `0 0 0 2px ${C.turk}, 0 8px 22px ${C.turk}22` : '0 1px 0 rgba(15,42,46,0.04), 0 8px 22px rgba(15,42,46,0.05)',
    }}>
      <div style={{ display: 'flex', alignItems: 'baseline', justifyContent: 'space-between', padding: '2px 4px 10px' }}>
        ...
        <span style={{ fontSize: 13, color: C.ink3, fontWeight: 500 }}>{date.getDate()}.{date.getMonth() + 1}.</span>
      </div>
      ...
      <div style={{ position: 'relative' }}>
          ...
          <div>
            ...
            <div style={{ fontSize: 22, fontWeight: 700, marginTop: 4, letterSpacing: -0.5 }}>
              {empty === total ? 'Aloitetaan viikko' :
                ...
                empty === 0 ? 'Viikko on valmis' : `${empty} ateriaa jäljellä`}
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {pending > 0 && (
            <button onClick={onAcceptAll} style={{
              ...
              <Icon name="check" size={16} color="#fff" /> Hyväksy {pending}
            </button>
          )}
        </div>
        ...
        {locked + pending > 0 && (
          <button onClick={onClear} style={{
            ...
            <Icon name="broom" size={13} color="rgba(255,255,255,0.78)" /> Aloita uusi viikko
          </button>
        )}
      </div>
    </div>
...
export default function WeekScreen({ plan, recipes, rolling, rollNames, onArvo, onOpen, onReroll, onArvoAll, onAcceptAll, onClear, family, monday, onBack, onAddToShopping }) {
  ...
  const todayIdx = isCurrent ? (today.getDay() + 6) % 7 : -1;
    ...
    <div style={{ paddingBottom: 100 }}>
      <div style={{ padding: '4px 16px 14px' }}>
        <button onClick={onBack} style={{
          height: 38, padding: '0 12px 0 8px', borderRadius: 12,
          background: 'transparent', border: 'none', cursor: 'pointer',
          display: 'inline-flex', alignItems: 'center', gap: 4,
          color: C.turkDeep, fontFamily: 'inherit', fontSize: 14, fontWeight: 600,
          marginBottom: 4, marginLeft: -4,
        }}>
          <Icon name="chevLeft" size={18} stroke={2.2} color={C.turkDeep} />
          Viikot
        </button>
        <div style={{ fontSize: 12, fontWeight: 700, color: C.turkDeep, letterSpacing: 0.8, textTransform: 'uppercase' }}>
          Viikko {isoWeek(monday)}{isCurrent ? ' · kuluva' : ''}
        </div>
        <div style={{ fontSize: 28, fontWeight: 800, color: C.ink, marginTop: 4, letterSpacing: -0.7, lineHeight: 1.1, textTransform: 'capitalize' }}>
          {rangeLabel(monday)}
        </div>
        <div style={{ fontSize: 13.5, color: C.ink2, marginTop: 4, fontWeight: 500, display: 'inline-flex', alignItems: 'center', gap: 6 }}>
          <Icon name="users" size={13} /> {family}
        </div>
      </div>

      <HeroCard stats={stats} onArvoAll={onArvoAll} onAcceptAll={onAcceptAll} onClear={onClear} />

      {(stats.locked + stats.pending) > 0 && (
        <div style={{ padding: '0 16px 14px' }}>
          <button onClick={onAddToShopping} style={{
            width: '100%', height: 50, borderRadius: 14, border: `1.5px solid ${C.turk}`,
            background: '#fff', color: C.turkDark, fontWeight: 700, fontSize: 15,
            fontFamily: 'inherit', cursor: 'pointer',
            display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: 8,
          }}>
            <Icon name="cart" size={18} color={C.turkDark} /> Lisää viikon ainekset ostoslistaan
          </button>
        </div>
      )}
